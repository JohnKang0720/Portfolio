...
An agent that works in a demo and an agent you can ship are different animals. The moment real users touch it, you find the long tail: the SQL it botches on a gnarly join, the answer that's *almost* right, the tone that drifts. Manual QA doesn't scale — every prompt tweak silently regresses three things you already fixed.

So I built the whole thing as a closed loop: the agent's own production traffic becomes the fuel for improving it. Here's the pipeline end to end.

## 1 · Capture everything (observability)

Every run is traced. The agent loop writes a **trace** per request and a **span** per tool call — the SQL it generated, the rows it got back, tokens, latency, which skill fired — into Snowflake (\`LLM_TRACES\` / \`LLM_SPANS\`). On top of that, each Slack answer carries a **👍 / 👎** button. Traces tell me *what the agent did*; the thumbs tell me *whether anyone was happy*. Together they're the raw material for everything downstream — no guessing which answers to look at.

## 2 · Define "correct" (reference data + a semantic model)

You can't score an answer without ground truth. Two pieces give it:

- **A semantic model in YAML** — the metrics, entities, and joins defined once (\`active_employees\`, \`gross_ggr\`, how tables link). It grounds the text-to-SQL agent so a term always means the same thing, and it's the schema the judge checks the agent's SQL against.
- **Reference answers generated by Claude** — for each eval question, Claude drafts a *golden* query + expected result against that semantic model. That gives the judge something real to grade against instead of its own opinion.

## 3 · Judge against a rubric

A separate **LLM-as-a-Judge** scores each response on named dimensions — correctness (vs. the reference), grounding, completeness, tone — and returns a score *and* a written critique per dimension. The 👎'd answers get pulled in with extra weight, so real user pain steers the eval.

> A scalar "it got worse" is useless; "grounding dropped 12 points on payroll questions" points straight at retrieval. The rubric turns a vibe into a diagnosis.

## 4 · Diagnose & refine

Critiques are clustered into **systematic failure modes**, not one-off misses. The dominant ones feed a prompt-rewriting step that proposes targeted edits to the system prompt and few-shot references — and can regenerate reference data where the goldens themselves were weak.

## 5 · Gate

A candidate prompt only ships if it **beats the incumbent on the full suite**. No regression, no merge — the same bar as code.

## Guardrails

- **Judge drift** — if the judge shares the agent's model and prompt, it rewards its own blind spots. I pin it to a different configuration and spot-check against human labels.
- **Overfitting the suite** — a rotating, held-out slice of real (thumbs-labelled) questions keeps the agent honest instead of teaching-to-the-test.

## End to end

Trace → feedback → reference data → judge → refine → gate → back into production, where the next traces are already accruing. Evaluation isn't the thing you do *after* building the agent; wired back in like this, it **is** the agent's improvement engine.`,
  },
  {
    id: 'databot-to-dbt',
